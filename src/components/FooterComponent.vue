<script setup>
  import '@/assets/styling/footerStyle.css'
</script>

<template>
  <footer>
    <div class="container">
      <div class="column column1">
        <img src="../images and icons/GH til Webudvikling/Ikoner/gh_logo_vertikal_hvid.svg" alt="Geografisk have logo footer" class="GH_logo">

      </div>
      <div class="column column2">
        <h3>Om os</h3>
        <a href="">Bestyrelsen</a>
        <a href="">Medarbejder</a>
        <a href="">Udviklingsplan</a>
        <a href="">Årsberetninger</a>
        <a href="">Samarbejdspartner</a>
        <a href="">Pressebilleder</a>
        <a href="">Presseperspektiv</a>
      </div>
      <div class="column column3">
        <h3>Diverse</h3>
        <a href="">Persondatapolitik</a>
        <a href="">Cookiepolitik</a>
        <a href="">Smiley kontrolrapport</a>
        <div class="social_media_div">
          <h3>sociale medier</h3>
          <img src="../images and icons/GH til Webudvikling/Ikoner/Facebook.svg" alt="">
          <img src="../images and icons/GH til Webudvikling/Ikoner/Instagram.svg" alt="">  
        </div>
      </div>
      <div class="column column3">
        <h3>Kontakt </h3>
        <p>Geografisk Have</p>
        <p>Christian 4 vej 23</p>
        <p>6000 Kolding</p>
        <div class="info_div">
          <p>79797000</p>
          <p>info@geografiskhave.dk</p>
        </div>
      </div>
    </div>
    <div class="bambus_footer">
      <img src="../images and icons/GH til Webudvikling/Ikoner/bambus_footer.png" alt="">
    </div>
  </footer>
</template>
...
const unwrapSignupData = ({ name, regionalCode, phone, email, numberOfParticipants }) => {
    return {
        name: name.value,
        ...
        <div class="event-registration-form">
            <form @submit.prevent="debounceSubmitForm">
                <fieldset class="form-input-required">
                    <input id="input-name" :class="{ required: (formData.name.required && formData.name.invalid) }"
                        type="text" placeholder="Navn" name="name" v-model="formData.name.value" required />
                    <label for="input-name" :class="{ show: formData.name.value.length > 0 }">Navn</label>
                </fieldset>

                <fieldset class="form-phone-fieldset">
                    <fieldset class="form-input-required">
                        <input id="input-regionCode"
                            :class="{ required: (formData.regionalCode.required && formData.regionalCode.invalid) }"
                            type="text" placeholder="Landekode" name="regionCode" v-model="formData.regionalCode.value"
                            required />
                        <label for="input-regionCode"
                            :class="{ show: formData.regionalCode.value.length > 0 }">Landekode</label>
                    </fieldset>
                    <fieldset class="form-input-required">
                        <input id="input-phone"
                            :class="{ required: (formData.phone.required && formData.phone.invalid) }" type="text"
                            placeholder="Telefon nummer" name="phone" v-model="formData.phone.value" required>
                        <label for="input-phone" :class="{ show: formData.phone.value.length > 0 }">Telefon
                            nummer</label>
                    </fieldset>

                    <p v-show="formData.regionalCode.errorMessage" class="form-error-message">
                        {{ formData.regionalCode.errorMessage }}</p>
                ...
                <fieldset class="form-input-required">
                    <input id="input-email" :class="{ required: (formData.email.required && formData.email.invalid) }"
                        type="email" placeholder="E-mail" name="email" v-model="formData.email.value" required>
                    <label for="input-email" :class="{ show: formData.email.value.length > 0 }">E-mail</label>
                ...
                <fieldset class="form-input-required">
                    <input id="input-amount"
                        :class="{ required: (formData.numberOfParticipants.required && formData.numberOfParticipants.invalid) }"
                        type="text" name="amount" placeholder="Antal" v-model="formData.numberOfParticipants.value"
                        required>
                    <label for="input-amount"
                        :class="{ show: formData.numberOfParticipants.value.length > 0 }">Antal</label>

                    <p v-show="formData.numberOfParticipants.errorMessage" class="form-error-message">
                ...
                <fieldset class="form-input-checkbox">
                    <fieldset>
                        <input id="input-newsletter"
                            :class="{ required: (formData.newsletter.required && formData.newsletter.invalid) }"
                            type="checkbox" name="newsletter" v-model="formData.newsletter.value" />
                        <label for="input-newsletter">Tilmeld vores nyhedsbrev</label>
                    </fieldset>

                    <fieldset>
                        <input id="input-gdpr" :class="{ required: (formData.gdpr.required && formData.gdpr.invalid) }"
                            type="checkbox" name="gdpr" ref="gdpr" v-model="formData.gdpr.value"
                ...
                <p v-show="generalErrorMessage" class="form-error-general-message">{{ generalErrorMessage }} </p>
